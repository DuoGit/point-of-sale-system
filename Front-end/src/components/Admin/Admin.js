import React, {Fragment} from 'react'
import Transaction from '../Transaction/Transaction'
import Inventory from '../Inventory/Inventory'
import BarcodeAdmin from '../Barcode/BarcodeAdmin'
import User from '../User/User'

import {Tabs, Tab} from 'material-ui/Tabs';
import {withRouter} from 'react-router-dom'
import BarcodeLogo from './BarcodeLogo' 
import GoodsLogo from 'material-ui/svg-icons/maps/local-grocery-store'
import HistoryLogo from 'material-ui/svg-icons/action/description'
import LogoutLogo from 'material-ui/svg-icons/action/power-settings-new'
import AccountLogo from 'material-ui/svg-icons/action/account-balance'
import FlatButton from 'material-ui/FlatButton';

const logoutStyle={
  width: '35px',
  height: '35px',
}
class Admin extends React.Component {
  
    logoutUser = () => {
        this.props.logoutUser()
        this.props.history.push('/')
    }

    render(){
    return (
      <Fragment>
      <div className='admin-header'>
      <h2>ADMIN PANEL</h2>
      <FlatButton onClick={this.logoutUser} primary={true}><LogoutLogo style={logoutStyle}/></FlatButton>
      </div>
      <Tabs>
      <Tab icon={<GoodsLogo/>} label="Inventory Management">
        <Inventory/>
      </Tab>
      <Tab icon={<BarcodeLogo/>} label="Barcode Scanner">
        <BarcodeAdmin/>
      </Tab>
      <Tab icon=  {<HistoryLogo/>} label="Transactions History">
        <Transaction/>
      </Tab>
      <Tab icon={<AccountLogo/>}label="Shop Management">
        <User/>
      </Tab>
    </Tabs>     
    </Fragment>
    )
  }
}

export default withRouter(Admin)